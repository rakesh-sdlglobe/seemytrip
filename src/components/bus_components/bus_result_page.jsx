...
import { fetchBusSearch, fetchBusSeatLayout } from '../../store/Actions/busActions';
import { selectBusSearchList, selectBusSearchLayoutList } from '../../store/Selectors/busSelectors';
  ...
  const filteredResults = busResults.filter((bus) => {
    if (
      ...
      !filters.busTypes.some(type =>
        bus.BusType &&
        bus.BusType.toLowerCase().replace(/[\s\-\/]/g, '')
          .includes(type.toLowerCase().replace(/[\s\-\/]/g, ''))
      )
    ) return false;

    const price = bus.BusPrice?.PublishedPriceRoundedOff || 0;
    if (price < (filters.priceMin || 0) || price > filters.priceMax) return false;
  ...
  return (
    <div className="col-xl-9 col-lg-8 col-md-12">
      ...
      <div className="row align-items-center g-2 mt-2">
        {loading ? (
          <>
            <div className="col-xl-12"><ResultSkeleton /></div>
            <div className="col-xl-12"><ResultSkeleton /></div>
            <div className="col-xl-12"><ResultSkeleton /></div>
          </>
        ) : (
          <>
            {filteredResults.map((bus, index) => {
              ...
              return (
                <div className="border rounded p-3 mb-3 bg-white" style={{ borderColor: '#007bff' }} key={index}>
                  <div className="d-flex justify-content-between align-items-start flex-wrap">
                    ...
                    <div>
                      ...
                      <div className="d-flex align-items-center mb-2">
                        <span className="bg-primary text-white px-2 py-1 rounded me-2" style={{ fontSize: '0.8rem' }}>★ 4.2</span>
                        <span className="text-muted" style={{ fontSize: '0.9rem' }}>103 Reviews</span>
                      </div>
                      <button className="btn btn-outline-secondary btn-sm px-3 py-1">On Time</button>
                    </div>

                    {/* Middle (Aligned Departure - Duration - Arrival) */}
                    <div className="d-flex align-items-center justify-content-center mx-auto" style={{ minWidth: 300 }}>
                      <div className="text-center px-3" style={{ minWidth: 100 }}>
                    ...
                    <div className="text-end">
                      <div className="fw-bold fs-4 mb-3">₹{bus.BusPrice?.PublishedPriceRoundedOff || '-'}</div>
                      <div className="text-muted mb-2">{bus.AvailableSeats} Seats Left</div>
                      <button
