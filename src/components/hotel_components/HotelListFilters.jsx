...
import { Link } from 'react-router-dom';
  ...
  const getMealPlanName = (meal) => {
    if (meal === "BB") return "Bed & Breakfast";
    if (meal === "HB") return "Half Board";
    if (meal === "FB") return "Full Board";
    if (meal === "AI") return "All Inclusive";
    if (meal === "RO") return "Room Only";
    return meal;
  };
  ...
  const HotelsFilters = ({ filters, TotalHotel,selectAmenity,selectMeal,selectLocalities,selectStarRatings , onMealFilterChange,onAmenityFilterChange,onLocalitiesFilterChange,onStarRatingsFilterChange,selectPrice,onFilterChange, onClearAll }) => {
    ...
    // State to keep track of previous filters
    // const [prevFilters, setPrevFilters] = useState({
    //   meal: selectMeal,
    //   amenity: selectAmenity,
    //   locality: selectLocalities,
    //   rating: selectStarRatings,
    //   price: selectPrice
    // });
    ...
    //       ...(currentFilters.meal && { mealPlan: currentFilters.meal }),
    //       ...(currentFilters.amenity && { amenities: currentFilters.amenity }),
    //       ...(currentFilters.locality && { localities: currentFilters.locality }),
    //       ...(currentFilters.rating && { starRatings: currentFilters.rating }),
    //       ...(currentFilters.price && { priceRange: currentFilters.price }),
    //     };

    //     // Dispatch fetchHotelsList with the current filters
    //     // if (fetchHotelsList) {
    //     //   fetchHotelsList(appliedFilters);
    //     // }
