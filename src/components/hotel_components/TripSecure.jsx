  ...
  >
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <img
          src="https://imgak.mmtcdn.com/pwa_v3/pwa_commons_assets/desktop/insurance/icici_logo.png"
          alt="ICICI"
          style={{ height: 32 }}
        />
        <img
          src="https://imgak.mmtcdn.com/pwa_v3/pwa_commons_assets/desktop/insurance/health_logo.png"
          alt="Health"
          style={{ height: 32 }}
        />
      </div>
    ...
    >
      <ul style={{ listStyle: "none", padding: 0, margin: 0, fontSize: 15 }}>
        <li style={{ marginBottom: 8 }}>
          ...
          <span style={{ float: "right", color: "#888", fontSize: 13 }}>
            {"INR "}
            {pricedetails.HotelServices[0].ServicePrice.toLocaleString()}
          </span>
        </li>
        {hotelServiceTax &&
          hotelServiceTax.TaxSummary &&
          hotelServiceTax.ServiceDetails.map((ser, index) => (
            <>
              <span style={{ display: "none" }}>
                {(totalPrice += ser.ServiceBookPrice)}
              </span>
            </>
          ))}
        {hotelServiceTax &&
          hotelServiceTax.TaxSummary &&
          hotelServiceTax.TaxSummary.map((tax, index) => (
            <>
            {tax.TaxValue > 0 && (<>
            <span style={{ display: "none" }}>
                {(totalPrice += tax.TaxValue)}
              </span>
              <li style={{ marginBottom: 8 }}>
                {tax.TaxName}{" "}
                <span style={{ float: "right", color: "#888", fontSize: 13 }}>
                  {"INR "}
                  {tax.TaxValue.toLocaleString()}
                </span>
              </li>
            </>)}
    ...
    >
      <div style={{ marginBottom: 8 }}>
        <input
          ...
          style={{ marginRight: 8 }}
        />
        <label htmlFor="secureYes" style={{ fontSize: 15 }}>
          Yes, secure my trip.
        </label>
      </div>
      <div>
        <input
          ...
          style={{ marginRight: 8 }}
        />
        <label htmlFor="secureNo" style={{ fontSize: 15 }}>
